...
[§12.21.1](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12211-general) says an event access shall only appear as the left hand operand of `+=` or `-=`. The existing Roslyn compiler accepts `e.E = h` and `e.E ??= h` on a field-like event when the synthesized backing field is accessible from the call site (typically from within the declaring type), because at that point the event access is treated as a field access via the generated backing field. This relaxation is reachable from both `new C { E = h }` (which was already legal pre-feature in the same positions) and, under this proposal, from `new C { E ??= h }`.
