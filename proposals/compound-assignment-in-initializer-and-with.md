...
An object initializer consists of a sequence of member initializers, enclosed by `{` and `}` tokens and separated by commas. Each *member_initializer* shall designate a target for the initialization. An *identifier* shall name an accessible field or property **or event** of the object being initialized, whereas an *argument_list* enclosed in square brackets shall specify arguments for an accessible indexer on the object being initialized. **The legality of an event target under a given *assignment_operator* follows from the lowering below: it is legal iff the equivalent *statement_expression* would be. In particular, `+=` and `-=` on an event target are always legal ([§12.21.5](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12215-event-assignment)); other *assignment_operators* that read the event require an accessible backing field (see [Event targets](#event-targets) below).** ~~It is an error for an object initializer to include more than one member initializer for the same field or property.~~ **For any given field, property, or event target, at most one member initializer may use the `=` operator. Any number of member initializers using a *compound_assignment_operator* are permitted for the same target. If both are present for the same target, the `=` member initializer shall appear in lexical order before any *compound_assignment_operator* member initializer for that target. No such restriction applies to indexer targets.**

*Note*: While an object initializer is not permitted to set the same field, property, or event more than once with `=`, the existing "same indexer arguments multiple times" allowance is preserved. The relaxation above for compound operators supports, among other things, subscribing multiple handlers to the same event in a single initializer (`Click += h1, Click += h2`) and accumulating into the same property (`Value = 10, Value += 5`). *end note*
...
On the right hand side of the `with` expression is a `member_initializer_list` with a sequence of assignments to *identifier*, which must be an accessible instance field or property **or event** of the receiver's type. **The legality of an event target under a given *assignment_operator* follows from the lowering below: it is legal iff the equivalent *statement_expression* on the clone would be. In particular, `+=` and `-=` on an event target are always legal; other *assignment_operators* that read the event require an accessible backing field (see [Event targets](#event-targets) below).**

**For any given field, property, or event target, at most one member initializer may use the `=` operator. Any number of member initializers using a *compound_assignment_operator* are permitted for the same target. If both are present for the same target, the `=` member initializer shall appear in lexical order before any *compound_assignment_operator* member initializer for that target.**
...
- **Event targets.** <a id="event-targets"></a> On an event target, `+=` and `-=` dispatch to the event's `add` and `remove` accessors respectively, per [§12.21.5](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12215-event-assignment), and are unconditionally legal. A single object initializer or `with` expression may subscribe or unsubscribe multiple handlers on the same event in lexical order. Any *assignment_operator* other than `+=` / `-=` reads the event's backing field, so its legality is governed by [§12.21.1](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12211-general)'s rule that "*an event access shall only appear as the left hand operand of `+=` or `-=`*." In the existing Roslyn implementation, that rule is relaxed for `=` and `??=` when the event is field-like and its compiler-synthesized backing field is accessible from the initializer's call site: `new C { E = h }` and `new C { E ??= h }` compile and execute when (and only when) `new C().E = h` or `c.E ??= h` would compile outside an initializer. When the backing field is **not** accessible (custom events, or field-like events accessed from outside the declaring type), both the initializer and the equivalent *statement_expression* reject with CS0070 / CS0079. This matches the behavior of the equivalent *statement_expression* and the "at most one `=` ... before any compound" uniqueness rule above, so the two forms stay consistent. See [Open LDM question: formalize `=` / `??=` on field-like events](#does-the-spec-allow---on-field-like-events). Other *compound_assignment_operators* on an event target are rejected — from outside the declaring type via CS0070 (they would read an inaccessible backing field), and from inside the declaring type via CS0019 through delegate overload resolution — because [§12.21.4](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12214-compound-assignment) does not define `*= / /= / ...` against an event access as its left operand.

- **Indexer targets.** The grammar of [§12.8.16.3](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#128163-object-initializers) continues to permit `'[' argument_list ']'` as an *initializer_target* for both the `=` form and the *compound_assignment_operator* form. The "arguments shall always be evaluated exactly once" rule already specified for indexer initializer targets in [§12.8.16.3](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#128163-object-initializers) is unchanged and applies equally to compound member initializers and to `??=` member initializers (the cached arguments are reused for the get and for the possibly-elided set); the get-and-set requirement from [§12.21.4](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12214-compound-assignment) applies to the selected indexer. An indexer target is exempt from the "at most one `=`, `=` before any compound" uniqueness rule above — any number of `=` or compound initializers may target the same indexer regardless of argument values.
...
### Does the spec allow `=` / `??=` on field-like events? <a id="does-the-spec-allow---on-field-like-events"></a>

[§12.21.1](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12211-general) says an event access shall only appear as the left hand operand of `+=` or `-=`. The existing Roslyn compiler accepts `e.E = h` and `e.E ??= h` on a field-like event when the synthesized backing field is accessible from the call site (typically from within the declaring type), because at that point the event access is treated as a field access via the generated backing field. When the backing field is **not** accessible — a custom event, or a field-like event accessed from outside the declaring type — the compiler rejects `=` and `??=` with CS0070 / CS0079, matching what the equivalent *statement_expression* form reports in the same positions. This relaxation is reachable from both `new C { E = h }` (which was already legal pre-feature in the same positions) and, under this proposal, from `new C { E ??= h }`.

Two plausible answers:

- **Formalize the Roslyn behavior.** Amend [§12.21.1](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12211-general) to say that an event access may also appear as the left hand operand of `=` or `??=` when its backing field is accessible at the call site. The initializer/`with` forms inherit the rule via the *statement_expression* lowering, and no separate event-specific carve-out is needed in this proposal.

- **Keep the stricter rule and change the compiler.** Treat `e.E = h` and `e.E ??= h` as compile-time errors (per the current [§12.21.1](https://github.com/dotnet/csharpstandard/blob/standard-v7/standard/expressions.md#12211-general)), and have the initializer/`with` forms produce the same diagnostic. This is a behavior break on existing code that compiles today.

Recommendation: formalize the Roslyn behavior. Keeping the initializer form consistent with the existing non-initializer behavior is more valuable than a spec-accurate restriction that would break today's working programs.
